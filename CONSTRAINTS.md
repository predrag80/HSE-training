...
- The [Crafto Consulting demo](https://crafto.themezaa.com/consulting/) is the
  approved primary visual target for the Astro public frontend.
- Corresponding HSE Training sections must reproduce its visible design with
  high fidelity, including section composition, layout proportions, container
  widths, spacing, typography hierarchy, card and button dimensions, image
  placement, borders, radii, backgrounds, navigation, and approved interactions.
- Visual fidelity applies across mobile, tablet, laptop, and desktop behavior,
  including stacking, column collapse, font resizing, whitespace, image
  cropping, navigation changes, and card-grid changes.
- The visual result should match Crafto closely, but the content must be real,
  approved HSE Training content. Do not blindly reproduce Crafto copy or invent
  HSE services, qualifications, instructors, prices, clients, or outcomes.
- If a Crafto section has no valid HSE business purpose or lacks approved HSE
  content, record the discrepancy for owner review instead of forcing content
  into the layout.

#### Implementation independence

- Visual fidelity must not create implementation coupling to Crafto or
  Elementor internals.
- Do not copy Elementor DOM structure, Elementor-generated class names, Crafto
  CSS classes, WordPress shortcodes, Crafto CSS, WordPress templates, vendor
  JavaScript architecture, or animation scripts.
- Do not install Crafto or Elementor, use Crafto WordPress templates, or depend
  on Crafto assets that are not licensed and approved for this project.
- Do not reproduce unnecessary wrapper markup generated by page builders.
- Implement the visual target with Astro components, Astro layouts, semantic
  HTML, project-specific class names, project-owned CSS, CSS Grid/Flexbox, and
  minimal JavaScript where an approved interaction requires it.
- Prefer CSS and native browser APIs for visible interactions and animations. A
  new animation library requires a separately approved task and documented need.
- Preserve visual behavior while simplifying implementation structure.

Visual equivalence does not require DOM equivalence. Clean semantic Astro markup
is preferred even when the reference uses deeply nested page-builder markup,
provided the rendered result closely matches the approved reference.
...
- frontend design work includes comparison against the approved Crafto visual
  target at representative mobile, tablet, laptop, and desktop widths;
- any reference section omitted because it lacks a valid HSE purpose or approved
  content is documented for owner review;
